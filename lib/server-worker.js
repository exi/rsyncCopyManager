...
var Server = function(modelInstance) {
    console.error('starting server worker ' + modelInstance.hostname + ' ' + modelInstance.path);
    var api = {};
    var stop = false;
    var checkTimer;
        ...
        deps.database.get(function(err, models) {
            ...
            function fetchPack() {
                ...
                packChain.then(function() {
                        ...
                        models.FSEntry.findAll({
                            ...
                            matches.forEach(function(m) {
                                if (pmap.hasOwnProperty(m.path)) {
                                    ...
                                    console.error('ignore path ' + tf[idx].path);
                                }
                            });
                    ...
                    newChain.addBoth(function() {
                        if (stop) {
                            console.error('newChain addBoth stop');
                            return finishedFsCheck();
                        }
                        chainLength--;
            ...
            function insertOrUpdate(fse) {
                packFind(fse.path).then(function(d) {
                    if (stop) {
                        console.error('insertOrUpdate packFind stop');
                        return finishedFsCheck();
                    }
                    var match = d[0];
                    var p = d[1];
                    ...
                    handle.done(function(err) {
                        if (stop) {
                            console.error('packFind handle stop');
                            return finishedFsCheck();
                        }
                        if (err) {
                            console.error('packFind handle err');
                            console.error(err);
                            return p.reject(err);
                        }
                        change = true;
            ...
            rsyncp.on('finish', function() {
                fetchPack().then(function() {
                    ...
                    modelInstance.save(['fse_revision']).done(function(err) {
                        if (err) {
                            return p.reject(err);
                        }
                        deps.database.query(
