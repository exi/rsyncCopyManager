...
var Download = module.exports = function(deps, modelInstance) {
    ...
    function download(server) {
        ...
        var src = modelInstance.path;
        var dest = parts.join('/') + '/';

        var Downloader = providerHelper.getDownloader(server.type, deps);
        if (Downloader === null) {
            console.error('Downloader for server ' + server.id + ' not found!');
            serverOffline = true;
            finishToken();
            return resetDownloadTimer();
        }

        downloader = new Downloader(src, dest, server);
    ...
    function getDownloaderForServer(server) {
        //todo, update db
        var provider = providerHelper.getProvider(server.type);
        if (provider && provider.downloader) {
            return provider.downloader(deps);
        } else {
            throw new Error('downloader for type ' + server.type + ' not found');
        }
    }
